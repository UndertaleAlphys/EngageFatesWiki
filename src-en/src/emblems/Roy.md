# Roy ×5

## Stat Bonuses

- **Lv 1**：HP+3  Str+2
- **Lv 5**：HP+5  Str+3
- **Lv 10**：HP+7  Str+4
- **Lv 20**：HP+10  Str+5

## Sync Skills

- **Lv 1**：HoldOut
  - If unit had 30% HP or more at start
of combat, unit would not take
damage from the lethal attack.
After that, lose HoldOut in this turn.
- **Lv 1**：Terrain Resistance
  - Nullifies damage from terrain.
Unit won't be affected by the negative
effect of terrain or pay extra cost on
any terrain.
- **Lv 5**：Advance
  - Use to move up to 2 spaces toward a foe
that is up to 3 spaces away and attack.
- **Lv 5**：HoldOut+
  - If unit had 20% HP or more at start
of combat, unit would not take
damage from the lethal attack.
After that, lose HoldOut in this turn.
- **Lv 10**：Press Advantage
  - When the enemy is in
a negative effect terrain,
Dmg+5.
- **Lv 10**：HoldOut++
  - If unit had 10% HP or more at start
of combat, unit would not take
damage from the lethal attack.
After that, lose HoldOut in this turn.
- **Lv 20**：Press Advantage+
  - When the enemy is in
a negative effect terrain,
Dmg+7.
- **Lv 20**：HoldOut+++
  - If unit had 2HP or more at start
of combat, unit would not take
damage from the lethal attack.
After that, lose HoldOut in this turn.

## Engage Skills

- **Lv 1**：Rise Above
  - Grants Lvl+5.

## Engage Attack

- **Blazing Lion**
  - Use to attack foe. The 3 spaces wide
area of the foe and the 2 rows beyond
it are set on fire.
Sword only, 1 range only.
- **Blazing Lion (Dragon)**
  - Use to attack foe. The 3 spaces wide
area of the foe and the 2 rows beyond
it are set on fire.
Sword only, 1 range only.
[Dragon] Expands burning area.

## Engage Weapons

- **Lv 1**：Lancereaver: Range 1, Mt 7, Hit 80, Crit 5 / Unusual sword wielded by Emblem
Roy. Grants advantage vs. lances
but disadvantage vs. axes.
- **Lv 10**：Wyrmslayer: Range 1, Mt 6, Hit 70 / Sword wielded by Emblem Roy.
Effective: Dragon.
- **Lv 20**：Binding Blade: Range 1~2, Mt 11, Hit 95, Crit 10 / Famous sword wielded by Emblem
Roy. Grants Def/Res+2. Can strike
close or at range.
Effective: Dragon

## Inheritable Skills

- **Lv 1**：Terrain Resistance
  - Cost 500 | Nullifies damage from terrain.
Unit won't be affected by the negative
effect of terrain or pay extra cost on
any terrain.
- **Lv 1**：Sword Prowess Lv.1
  - Cost 100 | Grants Atk+1
when using swords,
fails at weapon disadvantage.
- **Lv 1**：Sword Prowess Lv.2
  - Cost 300 | Grants Atk+2
when using swords,
fails at weapon disadvantage.
- **Lv 5**：Advance
  - Cost 500 | Use to move up to 2 spaces toward a foe
that is up to 3 spaces away and attack.
- **Lv 5**：Sword Prowess Lv.3
  - Cost 750 | Grants Atk+3
when using swords,
fails at weapon disadvantage.
- **Lv 10**：Press Advantage
  - Cost 750 | When the enemy is in
a negative effect terrain,
Dmg+5.
- **Lv 10**：Rally Strength
  - Cost 1000 | Grants the unit "Rally" command,
use to +4 Str to all allies in 2 radius,
last for 1 turn.
- **Lv 10**：Sword Prowess Lv.4
  - Cost 1500 | Grants Atk+4 and Hit+5
when using swords,
fails at weapon disadvantage.
- **Lv 20**：Press Advantage+
  - Cost 1000 | When the enemy is in
a negative effect terrain,
Dmg+7.
- **Lv 20**：Sword Prowess Lv.5
  - Cost 3500 | Grants Atk+4 and Hit+5
when using swords,
fails at weapon disadvantage.
[Swordfaire]Grants Atk+5

## Proficiency

- **Lv 10**：Sword

